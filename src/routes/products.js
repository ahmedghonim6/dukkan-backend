...
router.post('/', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('products').insert([{ name, price, description: description||'', stock: stock||0, store_id: storeId, options: options||[], related_ids: relatedIds||[], images: req.body.images||[]  }]).select().single()
